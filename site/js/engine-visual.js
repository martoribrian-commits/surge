/**
 * Canvas field — coherence particles, breath ring, bilateral wash, progress arc.
 */
(function (global) {
  ...
  function drawBreathRing(ctx, input, cx, cy, minDim) {
    if (input.phase !== 'active' || input.heartbeat < 0.35) return;
    var breath = 0.5 + 0.5 * Math.sin(2 * Math.PI * BREATH_HZ * input.t);
    var baseR = minDim * lerp(0.28, 0.22, input.progress);
    var radius = baseR + breath * minDim * 0.04 * input.heartbeat;
    var alpha = 0.12 + 0.18 * input.heartbeat;
    ctx.strokeStyle = 'rgba(127, 168, 146, ' + alpha + ')';
  ...
  function drawProgressArc(ctx, input, cx, cy, minDim) {
    if (input.phase !== 'active' && input.phase !== 'interrupted') return;
    var arcR = minDim * lerp(0.38, 0.32, input.progress);
    var start = -Math.PI / 2;
    ...
    ctx.stroke();
    ctx.strokeStyle = 'rgba(245, 166, 35, ' + (0.15 + 0.25 * input.heartbeat) + ')';
    ctx.beginPath();
  ...
  function drawTouchRings(ctx, input, cx, cy, minDim) {
    ...
    var base = minDim * (input.phase === 'idle' ? 0.16 : 0.14);
    var rings = [
      { scale: 1, alpha: 0.14 },
      { scale: 1.18, alpha: 0.09 },
      { scale: 1.36, alpha: 0.05 },
    ];
    ...
    ctx.fillStyle = 'rgba(255, 255, 255, ' + (input.phase === 'idle' ? 0.85 : 0.55) + ')';
    ctx.beginPath();
  ...
  function draw(ctx, input) {
    ...
    drawBreathRing(ctx, input, cx, cy, minDim);
