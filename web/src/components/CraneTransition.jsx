...
/**
 * Completion handoff — three final pulses, gray fade, optional Crane transition.
 */
export default function CraneTransition({ durationInSeconds, completedFullCycle }) {
  ...
  const handleTransitionToCrane = async () => {
    if (isHandoffActive) return;
    setIsHandoffActive(true);

    try {
      const supabaseContext = await fetchCraneContext(sessionPayload.sessionId);
      navigate('/crane', { state: { supabaseContext } });
    } catch {
      navigate('/crane');
    } finally {
      ...
      <AnimatePresence>
        {phase === 'ready' && (
          ...
          >
            ...
            <motion.button
              type="button"
              onClick={handleTransitionToCrane}
